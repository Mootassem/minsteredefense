...
import ProduitListItem from 'src/view/produit/list/ProduitListItem';

function CommandLineListTable(props) {
  const [
    recordIdToDestroy,
          ...
          <thead className="thead">
            <tr>
              <TableColumnHeader className="th-checkbox">
                ...
                <TableColumnHeader
                  label={i18n(
                    'entities.commandLine.fields.product',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                  ...
                  name={'quantity'}
                  label={i18n(
                    'entities.commandLine.fields.quantity',
                  )}
                  align="right"
                />
                <TableColumnHeader
                  ...
                  name={'subTotal'}
                  label={i18n(
                    'entities.commandLine.fields.subTotal',
                  )}
                  align="right"
                />
            ...
            {!loading &&
              rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>
                    <ProduitListItem value={row.product} />
                  </td>
                  <td style={{ textAlign: 'right' }}>{row.quantity}</td>
                  <td style={{ textAlign: 'right' }}>{row.subTotal}</td>
                  <td className="td-actions">
                    <Link
                      className="btn btn-link"
                      to={`/command-line/${row.id}`}
                    >
                      {i18n('common.view')}
                    </Link>
                    {hasPermissionToEdit && (
...
export default CommandLineListTable;

...
import ProduitForm from 'src/view/produit/form/ProduitForm';
import ProduitService from 'src/modules/produit/produitService';

function ProduitFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await ProduitService.create(data);
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {i18n('entities.produit.new.title')}
            </h5>
            <button
              type="button"
              className="close"

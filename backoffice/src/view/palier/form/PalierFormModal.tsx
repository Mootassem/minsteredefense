...
import PalierForm from 'src/view/palier/form/PalierForm';
import PalierService from 'src/modules/palier/palierService';

function PalierFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await PalierService.create(data);
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {i18n('entities.palier.new.title')}
            </h5>
            <button
              type="button"
              className="close"
...
export default PalierFormModal;

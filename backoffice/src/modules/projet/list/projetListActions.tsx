...
import exporterFields from 'src/modules/projet/list/projetListExporterFields';
...
const projetListActions = {
  ...
  doToggleAllSelected() {
    return {
      type: projetListActions.TOGGLE_ALL_SELECTED,
    };
  },

  doToggleOneSelected(id) {
    return {
      type: projetListActions.TOGGLE_ONE_SELECTED,
      payload: id,
    };
  },
  ...
  doExport: () => async (dispatch, getState) => {
    try {
      ...
      dispatch({
        type: projetListActions.EXPORT_STARTED,
      });
      ...
      new Exporter(
        exporterFields,
        i18n('entities.projet.exporterFileName'),
      ).transformAndExportAsExcelFile(response.rows);

      dispatch({
        type: projetListActions.EXPORT_SUCCESS,
      });
    } catch (error) {
      Errors.handle(error);

      dispatch({
        type: projetListActions.EXPORT_ERROR,
      });
    }
  },
  ...
  doChangePagination: (pagination) => async (
    ...
    dispatch({
      type: projetListActions.PAGINATION_CHANGED,
      payload: pagination,
    });
  ...
  doFetchCurrentFilter: () => async (
    ...
    dispatch(projetListActions.doFetch(filter, rawFilter, true));
  },
